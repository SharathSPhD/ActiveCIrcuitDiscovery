...
export default {
  async fetch(request, env) {
    ...
    if (seg === '' || seg === 'favicon.ico') {
      return json({ service: 'acd-demo gateway', backend: (await env.KV.get('gateway_url')) ? 'configured' : 'unset' }, 200);
    }
    ...
    const key = request.headers.get('x-acd-key');
    if (key) fwd.set('x-acd-key', key);
