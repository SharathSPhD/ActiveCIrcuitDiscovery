...
say "4. proxy reaches the GPU from the public URL"
PH=$(curl -s -m 30 "$SITE/api/dgx/health")
case "$PH" in
  *'"status":"ok"'*) ok "/api/dgx/health ok: $(printf '%s' "$PH" | head -c 120)";;
  *'offline'*)       no "proxy says OFFLINE — tunnel URL in Vercel is stale. Run ./dgx-server/golive.sh";;
  *)                 no "unexpected: $PH";;
esac

say "5. a real episode streams end-to-end"
