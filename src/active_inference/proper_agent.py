...
class ProperActiveInferenceAgent(IActiveInferenceAgent):
    ...
    def _create_generative_model(self):
        ...
        self.A = obj_array([
            np.zeros((self.num_obs[0], self.num_states[0], self.num_states[1], self.num_states[2])),
            np.zeros((self.num_obs[1], self.num_states[0], self.num_states[1], self.num_states[2]))
        ])
        ...
        self.B = obj_array([
            np.zeros((self.num_states[0], self.num_states[0], self.intervention_types)),  # Component transitions
            np.zeros((self.num_states[1], self.num_states[1], self.intervention_types)),  # Importance transitions
            np.zeros((self.num_states[2], self.num_states[2], self.intervention_types))   # Intervention transitions
        ])
        ...
        for action in range(self.intervention_types):
            for s in range(self.num_states[1]):
                if action == 0:  # Ablation can reveal true importance
                    ...
                    self.B[1][:, s, action] = norm_dist_obj_arr(self.B[1][:, s, action:action+1]).flatten()
        ...
        self.C = obj_array([
            np.array([0.0, 0.2, 0.4, 0.6, 0.8]),  # Prefer higher effect magnitudes
            np.array([0.0, 0.3, 0.7])              # Prefer higher confidence
        ])
        ...
        self.D = obj_array([
            np.ones(self.num_states[0]) / self.num_states[0],  # Uniform over components
            np.ones(self.num_states[1]) / self.num_states[1],  # Uniform over importance
            np.array([0.6, 0.3, 0.1])                         # Prefer starting with ablation
        ])
    ...
    def generate_predictions(self) -> List[NovelPrediction]:
        ...
        for intervention_type in range(self.intervention_types):
            ...
            predicted_qs = [
                spm_dot(self.agent.B[f][:, :, intervention_type], current_qs[f]) 
                for f in range(len(current_qs))
            ]
            
            # Predict observations under predicted states
            predicted_obs = [
                smp_dot(self.agent.A[m], predicted_qs) 
