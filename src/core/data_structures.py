...
class CircuitFeature:
    ...
    def __post_init__(self):
        ...
        if self.layer_idx < 0:
            raise ValueError(f"layer must be non-negative, got {self.layer_idx}")
        ...
        if not self.semantic_description:
            if self.feature_source == "transcoder":
                self.semantic_description = f"Transcoder L{self.layer_idx}F{self.feature_id} ({self.component_type})"
            else:
                self.semantic_description = f"SAE L{self.layer_idx}F{self.feature_id}"
    ...
    @classmethod
    def from_sae_data(cls, feature_id: int, layer_idx: int, data: Dict[str, Any]) -> 'CircuitFeature':
