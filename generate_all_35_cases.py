...
# Load data
with open('results/authentic_master_workflow_20250919_183530/comprehensive_experiment_results.json', 'r') as f:
    data = json.load(f)

print(f'🔍 Analyzing {len(data["test_case_details"])} test cases...')

# Create layer plots directory
results_dir = Path('results/authentic_master_workflow_20250919_183530/layer_feature_plots')
results_dir.mkdir(exist_ok=True)

# Generate plots for ALL test cases
total_cases = len(data['test_case_details'])
for i in range(total_cases):  # ALL cases, not just 5
    test_case = data['test_case_details'][i]
    ...
    if i < 10 or i % 5 == 0:  # Show progress for first 10 and every 5th case
        print(f'✅ Generated plot for case {case_id}')

print(f'\n🎉 Layer/feature analysis plots generated for ALL {total_cases} cases in: {results_dir}')
print(f'📊 Each case plot shows:')
print('  - Layer-wise feature activation heatmaps')
print('  - Mean activation by layer with feature counts')
print('  - Top 8 feature activations per case')
