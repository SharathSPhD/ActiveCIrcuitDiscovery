...
propagation.  GemmaScope~\cite{team2024gemma} provides pre-trained transcoders for
Google's Gemma-2 model family.
...
POMDP agents with the full Active Inference formalism, including variational state
inference, policy evaluation via EFE, and Dirichlet parameter
learning~\cite{Pymdp2022}. The present work uses \texttt{pymdp} to implement the
circuit discovery agent.
...
connection by embedding a pymdp POMDP agent within an automated circuit discovery
procedure.
...
Active Inference addresses this gap by maintaining a posterior over candidate
feature states and selecting the intervention that, in expectation, most reduces
...
\begin{table}[t]
  ...
  \begin{tabular}{l c c c c}
    \toprule
    \textbf{Method} & \textbf{Uncert.} & \textbf{Adaptive} & \textbf{Gen.\ Mod.} & \textbf{Bayesian} \\
    \midrule
    \ACDC~\cite{Conmy2023}       & No  & Thresh. & No  & No \\
    \EAP~\cite{Syed2023}         & No  & No      & No  & No \\
    Causal Trace~\cite{Meng2022} & No  & No      & No  & No \\
    \textbf{\ACD{} (pymdp)}      & Yes & Yes     & Yes & Yes \\
    \bottomrule
  \end{tabular}%
  }
\end{table}
