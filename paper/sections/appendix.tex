...
computed as:
\begin{equation}
  D_{\text{KL}}(p_{\text{interv}} \| p_{\text{clean}}) =
  \sum_v p_{\text{interv}}(v) \log \frac{p_{\text{interv}}(v)}{p_{\text{clean}}(v)}
\end{equation}
...
\begin{table*}[t]
  ...
  \begin{tabularx}{\textwidth}{llXX}
    ...
    \midrule
    \texttt{circuit\_tracer\_backend.py} & LogitTarget attribute &
      \texttt{.token} attribute does not exist on \texttt{LogitTarget} &
      Changed to \texttt{.token\_str} (correct attribute name) \\
    \texttt{circuit\_tracer\_backend.py} & PruneResult handling &
      Assumed \texttt{prune\_result.graph} exists; \texttt{PruneResult}
      is a namedtuple with \texttt{node\_mask} and \texttt{edge\_mask} &
      Rewrote to apply masks to raw graph's adjacency matrix \\
    \texttt{intervention\_engine.py} & Model access &
      \texttt{model.model.cfg} fails because \texttt{ReplacementModel}
      inherits directly from \texttt{HookedTransformer} &
      Changed to \texttt{model.cfg} (direct access) \\
    \texttt{intervention\_engine.py} & Intervention method &
      Used \texttt{run\_with\_hooks} with residual stream hooks instead
      of transcoder-level intervention &
      Replaced with \texttt{feature\_intervention} API for correct
      causal effects \\
    \texttt{runner.py} & Fake validation data &
      \texttt{\_generate\_prediction\_test\_data()} used
      \texttt{np.random.beta()} and \texttt{np.random.normal()} &
      Removed; replaced with real measurements from
      \texttt{feature\_intervention} \\
    \texttt{pomdp\_agent.py} & Skip bias &
      POMDP agent selected ``skip'' action 90\% of the time due to
      miscalibrated action space &
      Replaced POMDP with custom ActiveInferenceSelector using
