...
\caption{Cumulative KL divergence over 20 intervention steps on IOI
  (Gemma-2-2B), averaged across 5 prompts.  See Table~\ref{tab:ioi}
  for Llama results.  The POMDP agent (red) discovers high-impact
  features earlier than the bandit baseline (orange), greedy ranking
  (blue), and random selection, approaching the oracle upper bound
  (dashed).}
\label{fig:cumkl}
\end{figure}
...
\caption{Oracle efficiency comparison across IOI and multi-step
  reasoning tasks (Gemma-2-2B).  On IOI, the bandit leads; on
  multi-step, greedy marginally leads.  The POMDP agent is
  competitive on both, outperforming random by 8--17 percentage
  points.}
...
\caption{Mean KL divergence across steering multipliers for concept
  prompts on Gemma-2-2B (10 features each).  See Table~\ref{tab:steering}
  for Llama results.  Higher multipliers produce larger KL divergence,
  with concept-dependent sensitivity.}
\label{fig:steering}
\end{figure}
...
\begin{itemize}
\item Gemma-2-2B activates approximately 12\,600 transcoder features per
  IOI prompt; Llama-3.2-1B activates approximately 8\,000.
  Of these, approximately 2\,200 (Gemma) and 1\,600 (Llama) survive
  pruning at the 80\% influence threshold.
\item On Gemma, causally important features span all 26 layers but
  concentrate in layers 0--6 and 24--25.  On Llama (16 layers), the
  distribution is more compressed, with early layers (0--4) dominant.
\item Attribution graph generation takes approximately 18\,s (Gemma)
  and 12\,s (Llama); each \texttt{feature\_intervention} call takes
