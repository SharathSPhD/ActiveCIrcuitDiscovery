...
while the set of distinct dominant tokens broadens from 5 to 11 and
...
(79.4\%), with EAP strongest (98.1\%).  On Llama, however, the result is
a genuine negative: POMDP-abl collapses to 9.3\%, below random (44.8\%)
...
($\gamma{=}16$, $\eta{=}1.0$, pragmatic weight $1.0$; efficiency 82.0\%
Gemma, 52.1\% Llama).}
...
(Section~\ref{sec:multistep}).  A notable contrast is that the heuristic
...
and locality priors add little there, whereas on Llama IOI the heuristic
...
No claim is made of beating EAP, which is the strongest ablation
baseline throughout.
