...
Novelty of uncertainty-guided adaptive search.
...
\emph{(C3) Validated evaluation.} The POMDP agent is compared against a
...
\emph{(C4) Causal controllability.} Feature steering experiments demonstrate
...
\emph{(C5) Reproducibility artifacts.} Three Google Colab notebooks, a Docker
...
components together enable significantly more sample-efficient circuit discovery
than exhaustive or gradient-based approaches.
...
\emph{H2 (Causal controllability).}  Scaling individual transcoder features
...
\emph{H3 (Discovery quality).}  The POMDP agent achieves oracle
