...
\item \textbf{Active Inference POMDP Agent.}  A multi-factor POMDP agent
  implemented with \texttt{pymdp}~\cite{Pymdp2022} maintains a generative
  model of the circuit structure. The agent performs variational state
  inference, evaluates candidate interventions via Expected Free Energy,
  and learns its observation model online from real intervention data.
...
Three hidden state factors capture distinct aspects of each
candidate feature:
\begin{itemize}
\item \textbf{Factor 0: Feature importance}
  $s_0 \in \{\text{negligible}, \text{low}, \text{moderate}, \text{high}\}$
  (4 levels). Represents the causal contribution of a feature to the
  model's output on the current prompt.
\item \textbf{Factor 1: Layer role}
  $s_1 \in \{\text{early}, \text{middle}, \text{late}\}$ (3 levels).
  Determined by the feature's position within the network
  (layers are divided into thirds).
\item \textbf{Factor 2: Causal influence}
...
hidden states:
\begin{itemize}
\item \textbf{Modality 0: KL divergence magnitude.} The KL divergence
  between the clean and intervened output distributions, discretised
  into 4 levels: $\{$negligible ($<10^{-4}$), small ($<10^{-3}$),
  medium ($<10^{-2}$), large ($\geq 10^{-2}$)$\}$.
\item \textbf{Modality 1: Activation magnitude.} The absolute activation
  value of the feature, discretised into 4 levels.
\item \textbf{Modality 2: Graph connectivity.} The sum of in-degree
  and out-degree of the feature in the attribution graph, discretised
  into 3 levels: $\{$sparse, moderate, dense$\}$.
\end{itemize}

\subsubsection{Actions}

Three intervention types are available:
\begin{itemize}
\item \textbf{Ablation}: set the transcoder feature activation to zero.
\item \textbf{Activation patching}: replace the feature activation with
  a reference value from a different prompt.
\item \textbf{Feature steering}: scale the activation by a multiplier
  $m \in \{0, 2, 5, 10\}$.
\end{itemize}
...
\begin{itemize}
\item $\mat{A}$: \textbf{Observation likelihood} $P(o_m \mid s_0, s_1, s_2)$
  for each modality $m$. Encodes domain-informed priors about how hidden
  feature states produce observable intervention effects. The A matrix
  is learned online via Dirichlet concentration updates from each
  observation.

\item $\mat{B}$: \textbf{Transition model} $P(s' \mid s, a)$.
  Factor~0 (importance) is controlled by the three actions;
  Factors~1 and~2 have identity transitions (intrinsic properties that
  do not change upon intervention).

\item $\mat{C}$: \textbf{Preference model} (log-prior over preferred
  observations). Higher KL divergence and higher activation are
  preferred, encoding the goal of finding causally important features.

\item $\mat{D}$: \textbf{Prior beliefs} over hidden states. Biased
  toward low importance (most features are not causally critical) and
  uniform over layer role.
\end{itemize}
...
\begin{multline}
  G(i, a) = \underbrace{\E_{q}\!\bigl[\log q(s_\tau | \pi) - \log q(s_\tau | o_\tau, \pi)\bigr]}_{\text{epistemic: information gain}} \\
  + \underbrace{\E_{q}\!\bigl[\log p(o_\tau) - \log q(o_\tau | \pi)\bigr]}_{\text{pragmatic: preference satisfaction}}
\end{multline}
...
\begin{enumerate}
\item \textbf{Infers posterior states.} The pymdp agent performs
  variational inference to update $q(s_0, s_1, s_2 \mid o_0, o_1, o_2)$.

\item \textbf{Updates the observation model.} The Dirichlet
  concentration parameters $\mat{p_A}$ are updated:
  \begin{equation}
...
\item \textbf{Steps the time index.} The agent advances its internal
  clock and resets the observation buffer for the next step.
\end{enumerate}
