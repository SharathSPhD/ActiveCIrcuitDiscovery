...
next belief update.  \RoneHl[R1.4 (Figure 1 clarity): arrows/lines cross and the workflow is hard to follow. Resolved: Figures 1 and 2 were re-authored as clean raster diagrams (fig1.png, fig2.png) with non-crossing routing and clearer spacing.]{Figure~\ref{fig:architecture} shows the end-to-end
...
\RtwoHl[R2.M3 (Candidate extraction bias): top-k-per-layer selection biases the candidate set. Resolved: procedure stated precisely and the resulting bias acknowledged as a limitation -- efficiency is measured within an already-pruned set, not over all model components (Discussion).]{Candidates are sampled across layers (up to $k$ per layer) to ensure
diversity.}
...
Activation patching replaces the feature activation with a
