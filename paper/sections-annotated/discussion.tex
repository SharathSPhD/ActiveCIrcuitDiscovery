...
The primary limitation concerns multi-step reasoning on Llama. \RoneHl[R1.6 (Llama multi-step underperformance): the agent trails the bandit here; treat it as a limitation, not an architectural explanation, with further discussion. Resolved: reported as a limitation with per-prompt diagnostics and a finer-bin remedy that does not close the gap.]{On Llama multi-step the
...
\RtwoHl[R2.M8/W1 (Comparative methodology - ACDC/EAP): run ACDC/EAP or document the omission. Resolved: a direct EAP attribution ranking is reported on the same backend; a like-for-like ACDC run would require an edge-level substrate and is documented here as a limitation, not as evidence of superiority.]{A second limitation concerns comparison scope and the absence of a direct ACDC
