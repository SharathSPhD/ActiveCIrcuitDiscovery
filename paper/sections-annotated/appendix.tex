...
Ablation (action~0) is an exploratory intervention that
...
Activation patching (action~1) replaces the feature
...
Feature steering (action~2) scales the feature activation
...
\RoneHl[R1.9 (B-matrix transition probabilities): the values appear arbitrary; justify them. Resolved: the diagonals are not tuned to a metric but encode a monotone transition-entropy ordering (ablation, then patching, then steering) reflecting intervention semantics; only the ordering matters, not the exact fractions.]{The three diagonals are not tuned to
...
The $0.01$ importance-scaling factor in
...
Because these thresholds are fixed rather than learned, their
...
\RtwoHl[R2.W4/M2 (Generative model under-specified): provide the actual A, B, C and D matrices and how they are constructed. Resolved: the complete A, B, C, D arrays and the Dirichlet concentration prior are inlined in this section (auto-dumped from the implementation) and also released as JSON.]{For full reproducibility this section lists the exact generative model used by the
...
\begin{equation}
  \eta = \frac{\sum_{t=1}^{B} \text{KL}^{\text{abl}}_{i_t^{\text{method}}}}
                             ...
                             \times 100\% \in [0,100]\%.
\end{equation}
Both numerator and denominator are ablation KL, so $\eta$ is bounded by
