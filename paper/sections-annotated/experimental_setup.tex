...
observations. \RtwoHl[R2.M6 (Baselines and comparative methodology): no simpler UCB baseline; POMDP-abl vs bandit nuances undiscussed. Resolved: a plain UCB1 baseline removes the bandit's engineered priors, and POMDP-abl vs bandit is compared across all tasks (Table 9).]{A \emph{plain UCB} baseline removes these engineered
...
policy from that of the steering action. \RoneHl[R1.7 (Action-matched baselines): random/greedy/bandit lacked steering variants; does the POMDP gain come from EFE selection or from steering producing higher KL? Resolved: Greedy+steer, Bandit+steer and Random+steer added on the common RCK scale, isolating selection from the steering action.]{Steering-enabled counterparts of the
...
\emph{Random+steer}.} \RtwoHl[R2.W6 (Action-space ablation): add a random-action ablation that replaces the EFE objective. Resolved: a Random-action baseline (uniform feature and uniform intervention type) is reported on the RCK scale (Table 3), isolating EFE-guided action selection.]{A \emph{Random-action} baseline that selects a feature uniformly
...
Random, Greedy, EAP, Bandit, UCB), \RoneHl[R1.2 (Oracle efficiency >100\%): values such as 1255\% are confusing for an upper bound; explain mathematically or rename. Resolved: ablation-only efficiency is bounded in [0,100]\% against the ablation oracle; the unbounded quantity is renamed RCK below.]{every selector and the oracle draw from the same
...
not a bounded efficiency, and can exceed $100\%$. \RtwoHl[R2.W3 (Misleading >100\% metric): rename to relative cumulative KL and use ablation-only POMDP vs the ablation oracle as the primary comparison. Resolved: renamed RCK and reported only as a relative amplification factor; bounded efficiency is the headline discovery-quality metric.]{This quantity is named the
...
reported as significant only when $p<0.05$ under the permutation test. \RtwoHl[R2.W2 (Statistical weakness): increase prompt sets to at least 30 or use bootstrapped CIs; accept that H1 is unsupported on Llama IOI. Resolved: bootstrap 95\% CIs (10,000 resamples) on every reported number; H1 assessed by paired t-test and an exact permutation test and explicitly not accepted on Llama IOI.]{H1 is not
accepted where it is not significant (for example, Llama IOI).}

\RtwoHl[R2.M1 (Discretisation and state-space design): importance/layer-role binning is unjustified with no sensitivity analysis. Resolved: a +/-20\% threshold sweep (Table 7) leaves the qualitative ordering intact, and a finer 6-bin layer-role variant is run for Llama multi-step.]{The discretisation sensitivity of the agent is assessed at $\pm 20\%$.} The agent discretises
continuous observations (KL, activation, connectivity) at fixed thresholds, and \RtwoHl[R2.M3 (0.01 prior-observation scaling): unexplained. Resolved: the 0.01 factor maps graph importance into the KL-prior range; explained here and in the appendix.]{maps
...
\RtwoHl[R2.M4 (EFE selection sensitivity): horizon-1 planning may miss synergies; sensitivity to the exploration weight and action precision unexplored. Resolved: hyperparameter sweep over action precision, learning rate, and pragmatic weight (Table 8) leaves bounded efficiency essentially unchanged; tree-search planning noted as future work.]{while independently varying the action precision ($\gamma \in \{4, 16, 64\}$), the
...
CUDA~12.8, aarch64).  \RtwoHl[R2.W5 (Reproducibility/hardware): the container targets aarch64; provide an x86\_64 image for a consumer GPU and report T4-vs-DGX variance. Resolved: a standard amd64/x86\_64 Docker image and a free-T4 run are provided with pinned requirements; relative orderings are hardware-independent.]{\texttt{Dockerfile.amd64} (with
...
\RtwoHl[R2.M7 (Implementation gaps): number of runs, seeds, Dirichlet initialisation and exact B matrices were missing. Resolved: seeds, trial counts, Dirichlet init (pA=10A), eta, gamma and the full B matrices are specified (appendix), and the runner exposes every swept parameter.]{Random baselines use \texttt{numpy.random.seed(42)}, while the Active Inference Selector is
