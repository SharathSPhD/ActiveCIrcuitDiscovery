...
Section~\ref{sec:rck}.  \RtwoHl[R2 minor (Table 2): high coefficient of variation; report median and quartiles. Resolved: Table 2 now reports per-prompt median and inter-quartile range [Q1, Q3] in addition to the mean for every method.]{Mean and median per-step KL and the
...
\RoneHl[R1.5 (Oracle KL in Tables 2 and 4): the oracle row reported only 100\% without the actual KL value. Resolved: the oracle per-step mean and cumulative KL are now reported, making the efficiency normalisation explicit.]{The efficiency denominator is the oracle's per-step mean KL, $0.000862$
...
\RtwoHl[R2 minor (Figure 4 'dominates' claim): on Llama the bandit tracks the POMDP closely, so revise the dominates claim. Resolved: 'dominates' removed; EAP is the strongest baseline and on Llama bandit and POMDP-abl are close.]{Gemma, EAP tracks the oracle most closely, with POMDP-abl, bandit and
...
agent recovers more circuit structure than ablation-based methods; \RoneHl[R1.8 (Steering 94/100 - discovery vs exploitation): is the high efficiency structure discovery or steering-KL exploitation? Resolved: discovery quality is measured only by the bounded ablation-only efficiency; steering amplification is reported separately as RCK and labelled as exploitation, not discovery.]{the
...
multipliers, \RoneHl[R1.3 (Causal claims/steering controls): 10x scaling may cause OOD effects; add smaller scaling factors and a random-feature comparison. Resolved: a dose-response sweep m in {0,1.5,2,3,5,10} with a matched random-feature control; the causal claim is softened to controllability.]{a matched random-feature control of 10 random
...
\RtwoHl[R2.M5 (Intervention engine/causal correctness): the 10x multiplier is aggressive and may reflect fragility, with no evidence it amplifies the intended concept. Resolved: a top-k token analysis shows large multipliers amplify coherent concept tokens rather than only degrading the model.]{A complementary view of the largest multipliers confirms that steering
...
accumulation about circuit structure.  \RtwoHl[R2.M1 (Online observation-model learning): does the Dirichlet-learned A matrix actually converge within the budget? Resolved: the mean per-step L1 drift of the learned KL-likelihood matrix decreases monotonically and roughly halves over the budget (Figure 9), confirming convergence.]{The online learning of the
