...
%DIF ADD paper/paper.tex           Fri May 29 12:09:53 2026
...
%DIF < \newcommand{\ACD}{ACD}
%DIF -------
\newcommand{\ACDC}{\textsc{acdc}} %DIF > 
\newcommand{\EAP}{\textsc{eap}} %DIF > 
\newcommand{\ACD}{\textsc{acd}} %DIF > 
%DIF -------
...
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth]{figures/fig2.png}
...
\DIFdelbegin \DIFdel{In the single-step \ACD\ setting, each policy $\pi$ corresponds to a
single }\DIFdelend \DIFaddbegin \DIFadd{At each step, the agent evaluates each unobserved }\DIFaddend candidate feature $i$
and \DIFdelbegin \DIFdel{intervention type }\DIFdelend \DIFaddbegin \DIFadd{each action }\DIFaddend $a$ \DIFdelbegin \DIFdel{; substituting
$\pi = (i, a)$ into Equation~\ref{eq:efe} yields the following per-step EFE.
}\DIFdelend \DIFaddbegin \DIFadd{by computing the Expected Free Energy:
}\DIFaddend \begin{equation}
  \label{eq:efe_agent}
  \DIFdelbegin %DIFDELCMD < \EFE%%%
\DIFdelend \DIFaddbegin \DIFadd{G}\DIFaddend (i, a) = \underbrace{\mathbb{E}_{q}\!\bigl[\log q(s_\tau | \pi) - \log q(s_\tau | o_\tau, \pi)\bigr]}_{\text{epistemic}} + \underbrace{\mathbb{E}_{q}\!\bigl[\log p(o_\tau) - \log q(o_\tau | \pi)\bigr]}_{\text{pragmatic}}
\end{equation}

The candidate--action pair with the lowest EFE is selected:
\begin{equation}
  \DIFdelbegin %DIFDELCMD < \label{eq:efe_select}
%DIFDELCMD <   %%%
\DIFdelend (i^*, a^*) = \DIFdelbegin %DIFDELCMD < \argmin%%%
\DIFdelend \DIFaddbegin \DIFadd{\arg\min}\DIFaddend _{(i,a)} \DIFdelbegin \DIFdel{\; }%DIFDELCMD < \EFE%%%
\DIFdelend \DIFaddbegin \DIFadd{G}\DIFaddend (i, a)
\end{equation}
...
\DIFaddbeginFL \begin{tikzpicture}
\begin{groupplot}[
