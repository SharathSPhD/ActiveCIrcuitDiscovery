...
# Load data from ORIGINAL results (the good one)
results_path = 'results/authentic_master_workflow_20250919_174651'
with open(f'{results_path}/comprehensive_experiment_results.json', 'r') as f:
    data = json.load(f)

print(f'🔍 Analyzing ALL {len(data["test_case_details"])} test cases from ORIGINAL results...')

# Create layer plots directory
layer_plots_dir = Path(f'{results_path}/layer_feature_plots')
layer_plots_dir.mkdir(exist_ok=True)

# Generate plots for ALL 35 cases
for i, test_case in enumerate(data['test_case_details']):
    ...
    try:
        ...
        # Create comprehensive layer analysis plot
        ...
        if activations_matrix:
            ...
            if n_features <= 20:  # Only show labels if not too many
        ...
        if all_activations:
            top_indices = np.argsort(all_activations)[-10:]
            ...
            ax3.set_title('Top 10 Feature Activations')
        ...
        case_plot_path = layer_plots_dir / f'case_{case_id:02d}_layer_feature_analysis.png'
        plt.savefig(case_plot_path, dpi=300, bbox_inches='tight')
        plt.close()

        print(f'✅ Generated plot for case {case_id}')

    except Exception as e:
        print(f'❌ Error processing case {case_id}: {e}')

print(f'\n🎉 Complete layer/feature analysis plots generated in: {layer_plots_dir}')
print(f'📊 ALL 35 cases analyzed with:')
print('  - Layer-wise feature activation heatmaps')
print('  - Mean activation by layer with feature counts')
print('  - Top 10 feature activations per case')
